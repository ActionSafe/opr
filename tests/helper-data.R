...
gen_nhpp_ni = function(n_case, Lam_func, beta, X, max_time, levels = NULL, frailty = NULL, rho = 1) {
  ...
  # 生成Poisson过程（非齐次）
  nObs = sample(1:6, n_case, replace=TRUE)
  id = rep(1:n_case, nObs)
  sq = unlist(lapply(nObs, function(x){
    while(TRUE) {
      sq = unique(round(sort(runif(x, 0.1, max_time)), 1))
      if(length(sq)==x) break
    }
    sq
  }))
  Lam = Lam_func(sq) * rep(intensity, nObs)
  Lam = ((Lam+1)^rho - 1)/rho
  dLam = unlist(tapply(Lam, id, function(L) diff(c(0, L))))
  count = unlist(tapply(dLam, id, function(dL) rpois(n=length(dL), lambda = dL) ))
  ord = orderize(count, levels = levels)
  count_obs = ord$data
  qq = ord$levels
  lower = qq[count_obs]
  upper = qq[count_obs + 1]
  X = X[rep(1:n_case, nObs), ]
  data.frame(id, time = sq, count, count_obs, lower, upper, X)
}


gen_ord_nomix = function(n_case, beta, X, levels = NULL) {
  intensity = c(X %*% beta)
  # 生成Poisson过程（非齐次）
  id = 1:n_case
  # count = rlogis(n_case, location = intensity)
  count = rnorm(n_case, mean = intensity)
  ord = orderize(count, levels = levels)
  count_obs = ord$data
  # qq = ord$levels
  # lower = qq[count_obs]
  # upper = qq[count_obs + 1]
  data.frame(id, count, count_obs, X)
}

gen_ord_mix = function(n_case, beta, X, sig = 1, nObs = 5, levels = NULL) {
  intensity = c(X %*% beta)
  intensity = intensity + rnorm(n_case, sd = sig)
  intensity = rep(intensity, each = nObs)
  # 生成Poisson过程（非齐次）
  id = rep(1:n_case, each = nObs)
  sq = rep(1:nObs, n_case)
  # count = rlogis(n_case, location = intensity)
  count = rnorm(n_case*nObs, mean = intensity)
  ord = orderize(count, levels = levels)
  count_obs = ord$data
  # qq = ord$levels
  # lower = qq[count_obs]
  # upper = qq[count_obs + 1]
  X = X[id, ]
  data.frame(id, time = sq, count, count_obs, X)
}
